...
* 称拓扑空间 $X$ 为**胞腔复形**，如果其可以由如下方法构造出来：
  ...
  2. 设 $\{D_\alpha^n\}$ 是一族 $n$ 维球，$\phi_\alpha:\partial{D}_\alpha^n\to{X}^{n-1}$ 连续，将 $\phi_\alpha$ 视为贴映射，把所有 $D_\alpha^n$ 黏合到 $X^{n-1}$ 上，所得空间 $X^n$ 称为 **n-skeleton**，$D_\alpha^n$ 的内部 $e_\alpha^n$ 称为 **$n$ 维胞腔**.
