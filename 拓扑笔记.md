...
* 设 $p:\widetilde{X}\to{X}$ 是覆叠映射，我们称 $\widetilde{X}$ 的一个自同构为 **deck 变换**。全体 deck 变换构成群 $G(\widetilde{X})$，从而有群作用 $G(\widetilde{X})\curvearrowright\widetilde{X}:f\cdot\widetilde{x}=f(\widetilde{x})$。
  > 由提升的唯一性，如果在某个点处 $f\cdot\widetilde{x}_0=\widetilde{x}_0$，则必有 $f=\text{id}_{\widetilde{X}}$。

* 覆叠空间 $p:\widetilde{X}\to{X}$ 称为是**正规**的，如果对任意 $x\in{X}$ 和 $x$ 的提升 $\widetilde{x}_1,\widetilde{x}_2$，都存在某个 deck 变换将 $\widetilde{x}_1$ 映射到 $\widetilde{x}_2$。