...
* 称拓扑空间 $X$ 为**胞腔复形**，如果其可以由如下方法构造出来：
  ...
  2. 设 $\{D_\alpha^n\}$ 是一族 $n$ 维球，$\phi_\alpha:\partial{D}_\alpha^n\to{X}^{n-1}$ 连续。将 $\phi_\alpha$ 视为贴映射，把所有 $D_\alpha^n$ 黏合到 $X^{n-1}$ 上，所得的空间即为 $X^n$，也称 **n-skeleton**。$D_\alpha^n$ 的内部称为 **n-cell**.
  3. 若上述步骤在第 $n$ 步停止，令 $X=X^n$，称为 **$n$ 维胞腔复形**.
  4. 否则令 $X=\bigcup_{n}X^n$，并在其上装备弱拓扑，称为**无穷维胞腔复形**.

* 每个 cell 有特征映射 $\Phi_\alpha:D_\alpha^n\to{X}$，满足 $\Phi_\alpha|_{\partial_{D_\alpha^n}}=\phi_\alpha$，其定义为：$$D_\alpha^n\overset{\text{嵌入}}{\to}\bigsqcup_\beta{D_\beta^n}\sqcup{X}^{n-1}\overset{\text{黏合}}{\to}X^n\overset{\text{嵌入}}{\to}X$$
