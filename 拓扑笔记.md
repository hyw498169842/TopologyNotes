...
* 给定链复形的短正合列 $0.\to{A.}\overset{f.}{\to}B.\overset{g.}{\to}C.\to0.$，定义**边界映射** $\partial:H_n(C.)\to{H}_{n-1}(A.)$ 如下：设 $c\in{C}_n$ 是边缘链，选取 $b\in{B_n}$ 满足 $g_n(b)=c$。注意 $g_{n-1}\partial_n(b)=\partial_ng_n(b)=0$，因此 $\partial_n(b)\in\text{ker}(g_{n-1})=\text{Im}(f_{n-1})$。设 $a\in{A}_n$ 满足 $f_{n-1}(a)=\partial_n(b)$，我们定义 $\partial([c])=[a]$。
